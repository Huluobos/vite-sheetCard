<template>
  <div id="iframe-container">
    <process-nav active="4" :queryParam="data.queryParam"></process-nav>
...
<script setup lang='ts'>
  ...
  onMounted(() => {
    ...
    data.iframeSrc = data.rootUrl + "/newexam/marking/markingQuality?token=" + data.token
      +"&examId=" + data.queryParam.examId + "&subId=" + data.queryParam.subId + "&subName="+data.queryParam.subName
    window.onmessage = function (e) {
      if (e.data.source === 'markingQuality') {
