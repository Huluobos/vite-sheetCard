<template>
  <div id="iframe-container">
    <scan-detail-nav active="1" :queryParam="data.queryParam"></scan-detail-nav>
...
<script setup lang='ts'>
  ...
  onMounted(() => {
    ...
    data.iframeSrc = data.rootUrl + "/newexam/marking/selectQues?token=" + data.token
      +"&examId=" + data.queryParam.examId + "&subId=" + data.queryParam.subId + "&subName="+data.queryParam.subName
    window.onmessage = function (e) {
      if (e.data.source === 'selectQues') {
